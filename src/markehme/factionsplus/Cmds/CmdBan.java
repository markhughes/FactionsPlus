...
public class CmdBan extends FCommand {
	...
	public void perform() {
		...
		if(FactionsPlus.config.getBoolean("leadersCanFactionBan") && Utilities.isLeader(fme)) {
			authallow = true;
		} else if(FactionsPlus.config.getBoolean("officersCanFactionBan") && Utilities.isOfficer(fme)) {
			authallow = true;
		}
		...
		File banFile = new File(FactionsPlus.BASE_FOLDER + "fbans" + File.separator + pFaction.getId() + "." + banningThisPlayer.toLowerCase());
