...
public class Factions_CmdFactionsFaction extends FPCommand
{
	// -------------------------------------------- //
	// CONSTRUCT
	// -------------------------------------------- //
	
	public Factions_CmdFactionsFaction()
	{
		// Aliases
		this.aliases.add("f");
	...
	public void performfp()
	{
		...
		for (UPlayer follower : followers)
		{
			...
			{
				followerNamesOnline.add(follower.getNameAndTitle(usender) + Txt.parse("<aqua>(<green>" + String.valueOf(Math.ceil(follower.getPower())).replace(".0", "") +"<aqua>)<a>"));
			}
			else if (normal)
			{
				// For the non-faction we skip the offline members since they are far to many (infinate almost)
				followerNamesOffline.add(follower.getNameAndTitle(usender) + Txt.parse("<aqua>(<green>" + String.valueOf(Math.ceil(follower.getPower())).replace(".0", "") +"<aqua>)<a>"));
			}
		}
